...
#### 3.22 Numeric Variables Plot

```{r}
num_y <- train[, c('age', 'balance', 'day', 'duration', 'campaign', 'pdays', 'previous','y')]
ggpairs(num_y[,1:8], title = "Distribution & Correlation graph", ggplot2::aes(colour=y, alpha=1/100), upper = list(continuous = wrap("cor", size = 3, alignPercent = 1)))
```
...
#### 3.43 Job
```{r}
#Job + Term Deposit bar chart (count)
p11<-ggplot(train, 
           aes(x = job, 
               fill = y)) + 
...
#### 3.44 Age
```{r}
#Age + Term Deposit bar chart (count)
p12<-ggplot(train, 
           aes(x = age, 
               fill = y)) + 
  geom_bar(position = "dodge")+ scale_fill_manual(values=c("dodgerblue3", "dodgerblue4"))+ theme(axis.text.x = element_text(angle=90))+

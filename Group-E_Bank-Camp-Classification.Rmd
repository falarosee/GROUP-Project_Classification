...
cors <- cor(numerical)
corr_plot <- ggcorr(cors, label_round = 2, label = TRUE)
corr_plot
```
...
train_val<- function(train_dt, model, sampling){
  ...
  print("Cross Validation Scores having No as the positive class")
  print(model$results)
  print(now()-ini)
...
merged_df$Q3 <- merged_df$month_apr+merged_df$month_jul+merged_df$month_aug+merged_df$month_sep
